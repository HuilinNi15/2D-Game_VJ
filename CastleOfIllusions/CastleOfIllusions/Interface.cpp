...
GameScene::GameScene(bool ForestMap) : targetCameraY(cameraPos.y)
{
	...
	if (ForestMap)
		level = "levels/forest_map_prueba.txt";
	else
		level = "levels/chocolate_map_prueba.txt";

	isForestMap = ForestMap;
...
void GameScene::updateCam(int deltaTime)
{
	...
	//float deadZoneHeight = CAMERA_HEIGHT / DEAD_ZONE_SIZE;
	const int levelHeight = 192;
	const int transitionThreshold = 192; // Start scrolling down when player passes this y

	// Define horizontal dead zone boundaries
	...
	// Fixed camera behavior when player.y is 1344
	if (cameraPos.y == 1344) {
		// Set cameraPos.x to be fixed between 0 and 256
		cameraPos.x = glm::clamp(cameraPos.x, 0.0f, 256.0f);
	}
	else {
	...
	// Downward vertical scroll within the level
	if (posPlayer.y > transitionThreshold && posPlayer.y < levelHeight)
	{
		targetCameraY = posPlayer.y - (CAMERA_HEIGHT / 2);
	}
	else if (posPlayer.y >= levelHeight)
	{
		// Calculate the current level based on the player's y position
		int currentLevel = posPlayer.y / levelHeight;
		targetCameraY = levelHeight * currentLevel;
		posPlayer.y = (targetCameraY - 32); // Adjust player position for new level entry
	}

	// Gradually move cameraPos.y towards targetCameraY for smooth transition
	if (cameraPos.y < targetCameraY)
	{
		cameraPos.y += 400.0f * deltaTimeSec;  // Adjust the speed factor as needed
		if (cameraPos.y > targetCameraY)  // Prevent overshooting
	...
	if (cameraPos.y < 0)
		cameraPos.y = 0;
	else if (cameraPos.y + CAMERA_HEIGHT > mapHeight)
		cameraPos.y = (float)mapHeight - CAMERA_HEIGHT;

	projection = glm::ortho(cameraPos.x, cameraPos.x + float(CAMERA_WIDTH), cameraPos.y + float(CAMERA_HEIGHT), cameraPos.y);
}
...
void GameScene::update(int deltaTime)
{
	...
	lifeBarSprite->setPosition(glm::vec2(cameraPos.x, cameraPos.y + 160.f));
}
